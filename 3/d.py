...
class ConvolutionalNeuralNetworkModel(nn.Module):
    def __init__(self):
        ...
        self.relu = nnf.relu
        self.dropout = nnf.dropout
        self.dense = nn.Linear(64*7*7, 1024)
        self.dense2 = nn.Linear(1024, 10)

    def logits(self, x):
        x = self.pool(self.dropout(self.relu(self.conv(x)), p=0.1))
        x = self.pool(self.dropout(self.relu(self.conv2(x)), p=0.1))
        x = self.dense(x.view(-1, 64 * 7 * 7))
        return self.dense2(x.view(-1, 1024))
